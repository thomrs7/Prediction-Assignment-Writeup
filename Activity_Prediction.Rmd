...
The training data is loaded. Then nulls are removed as well unrelevant data i.e. timestamps, name, etc.
...
First we take a look at hostograms of the relevant data.
...
Upon inspection it looks las if many of the variabels have similar distrbutions and may be releated. To examin this further we look for data correlation.
...
We see the Random Forest is nearly  `r rf$overall[[1]]` accurate while the KNN is only  `r knn$overall[[1]]` accurate.  The Random Forest model will be used on the test data
